...
public:
    ...
    Lifes(glm::vec3 v1, glm::vec3 v2, float ySup, Shader& ourShader) {
        ...
        buttons.push_back(glm::vec4(0.085, 0.24, 0.45, 0.5));
        buttons.push_back(glm::vec4(0.26, 0.435, 0.45, 0.5));
        buttons.push_back(glm::vec4(0.455, 0.63, 0.45, 0.5));
...
private:
    ...
    void setUpWall(Shader& ourShader, int numLives) {
        ...
        unsigned char* data;
        string lives = "resources/Fotos_midi_maze/";
        if (numLives == 1) lives = lives + "1life.jpg";
        else if (numLives == 2) lives = lives + "2lives.jpg";
        else lives = lives + "3lives.jpg";
        
        data = stbi_load(lives.c_str(), &width, &height, &nrChannels, 0);
        if (data)
        {
