...
public:
    ...
    Speed(glm::vec3 v1, glm::vec3 v2, float ySup,float where, Shader& ourShader) {
        fast = true;
        vertices[0] = v1.x;
        vertices[1] = v1.y;
        vertices[2] = v1.z;
        ...
        cout << "DIM:" << ySup << endl;
        buttons.push_back(glm::vec4(0.406, 0.51, where, where + 0.0395));
        buttons.push_back(glm::vec4(0.532, 0.633, where, where +0.0395));
...
private:
    ...
    void setUpWall(Shader& ourShader, int fast) {
        ...
        if (fast == 0) img = img + "fast.jpg";
        else if (fast == 1) img = img + "slow.jpg";
